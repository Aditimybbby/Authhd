    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            if not attempt_token:
                return False, "Failed to extract checkout attempt token", gateway, total_price, currency
            ...
            vault_headers = {
                ...
                'Referer': f'https://checkout.pci.shopifyinc.com/build/{build_id or "a8e4a94"}/number-ltr.html?identifier=&locationURL=',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36 Edg/146.0.0.0',
                'sec-ch-ua': '"Chromium";v="146", "Not-A.Brand";v="24", "Microsoft Edge";v="146"',
                'sec-ch-ua-mobile': '?0',
                'sec-ch-ua-platform': '"Windows"',
            ...
            if response.status not in (200, 201):
                ...
                return False, f'Vault error {response.status}: {body[:200]}', gateway, total_price, currency
            try:
        ...
        return False, f"Error Processing Card: {type(e).__name__}: {str(e)}", gateway, total_price, currency
...
@app.route('/')
def root():
    return jsonify({"status": "ok", "service": "autoshopify"})
...
def shopify_checker():
    try:
        ...
        if variant_id and not re.fullmatch(r'\d+', variant_id):
            return jsonify({
                "error": "Invalid 'variant' parameter: must be numeric",
                "status": False
            }), 400
        ...
        try:
            ...
            return jsonify({
                "error": f"Async runtime error: {str(e)}",
                "status": False
            }), 500
